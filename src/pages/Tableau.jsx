...
import { 
  ...
  FiMessageSquare, FiSend, FiLoader, FiTrash2, FiFilter, FiEdit, FiSave, FiChevronDown
} from "react-icons/fi";
...
export default function Tableau({ changeTab, userRole }) {
  ...
  const [filterStatut, setFilterStatut]       = useState("");   // ← NOUVEAU
  ...
  // Note : filterStatut est géré côté client (voir filteredCommandes)
  ...
  const renvoyerBillet = async (cmd) => {
    ...
    try {
      ...
      const response = await fetch("/api/send-ticket-email", {
        ...
        body: JSON.stringify({ email: cmd.contact_email, firstName: cmd.contact_first_name, lastName: cmd.contact_last_name,
  ...
  const handleSendCustomMail = async (e) => {
    ...
    try {
      const response = await fetch("/api/send-custom-email", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: selectedOrder.contact_email, subject: customMailSubject,
  ...
  const exportToExcel = async () => {
    ...
    exportData.forEach(c => {
      ...
      const row = worksheet.addRow({
        ticket: c.ticket_num, client: `${c.contact_last_name} ${c.contact_first_name}`,
        phone: c.contact_phone, email: c.contact_email, sacrifice: c.sacrifice_name,
        ...
        acompte: `${(c.acompte_cents / 100).toFixed(2)} €`, paye: `${dejaPaye.toFixed(2)} €`,
        reste: `${reste.toFixed(2)} €`, statut: statutFr, stripe: c.stripe_ref || "",
      });
      if (rowColorArgb) {
  ...
  const getPaymentIcon = (moyen) => {
    switch(moyen) { case 'especes': return <FiDollarSign />; case 'cb': case 'stripe': return <FiCreditCard/>; default: return <FiFileText />; }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 pb-20 animate-fade-in">
      ...
      {/* ══════════════════════════════════════════════════════════════════════
          BARRE DE FILTRES — Jour · Créneau · Statut · Recherche · Reset
         ══════════════════════════════════════════════════════════════════════ */}
        ...
        <div className="flex flex-wrap gap-3 items-center">
          ...
          <div className="relative">
            <select
              ...
              className="appearance-none pl-3 pr-8 py-2.5 text-sm font-semibold bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-200 rounded-xl border-2 border-transparent focus:border-teal-500 outline-none cursor-pointer transition-all"
            >
              <option value="">📅 Tous les jours</option>
              {uniqueDates.map(date => (
                <option key={date} value={date}>
                  {getJourLabel(date)} — {new Date(date).toLocaleDateString('fr-FR')}
                </option>
              ))}
            </select>
          ...
          <div className="relative">
            <select
              ...
              className="appearance-none pl-3 pr-8 py-2.5 text-sm font-semibold bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-200 rounded-xl border-2 border-transparent focus:border-teal-500 outline-none cursor-pointer transition-all disabled:opacity-40"
            >
          ...
          <div className="relative">
            <select
              value={filterStatut}
              onChange={e => { setFilterStatut(e.target.value); setLimit(50); }}
              className="appearance-none pl-3 pr-8 py-2.5 text-sm font-semibold bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-200 rounded-xl border-2 border-transparent focus:border-teal-500 outline-none cursor-pointer transition-all"
            >
          ...
          <div className="relative flex-1 min-w-[200px] group">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 text-sm border-2 border-slate-200 dark:border-slate-600 rounded-xl dark:bg-slate-700 dark:text-white outline-none focus:border-teal-500 focus:ring-4 focus:ring-teal-500/10 transition-all font-medium"
            />
            {searchTerm && (
          ...
          {hasActiveFilters && (
            <button
              ...
              className="shrink-0 flex items-center gap-1.5 px-3.5 py-2.5 text-xs font-bold text-red-600 bg-red-50 dark:bg-red-900/20 hover:bg-red-100 dark:hover:bg-red-900/40 rounded-xl border border-red-200 dark:border-red-800 transition-colors"
            >
                  ...
                  {isEditing ? (
                    <div className="space-y-3">
                      ...
                      <div>
                        ...
                        <input type="text" value={editFormData.contact_phone} onChange={e => setEditFormData({...editFormData, contact_phone: e.target.value})} className="w-full p-2.5 text-sm border-2 rounded-xl dark:bg-slate-900 dark:border-slate-700 dark:text-white outline-none focus:border-amber-500" placeholder="06..." />
                      </div>
                      <div>
                        <label className="text-xs text-slate-500 mb-1 block">Email</label>
                        <input type="email" value={editFormData.contact_email} onChange={e => setEditFormData({...editFormData, contact_email: e.target.value})} className="w-full p-2.5 text-sm border-2 rounded-xl dark:bg-slate-900 dark:border-slate-700 dark:text-white outline-none focus:border-amber-500" placeholder="email@..." />
                      </div>
                    </div>
                  ) : (
                    <div>
                ...
                <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 space-y-4">
                  ...
                  {isEditing ? (
                    <div className="space-y-3">
                      ...
                      <p className="text-sm text-slate-500">Boucle : <strong className="text-emerald-600 font-black">{selectedOrder.numero_boucle || "En attente"}</strong> <span className="text-xs text-slate-400 italic">(Non modifiable ici)</span></p>
                      <div>
                ...
                <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 space-y-4">
                  ...
                  {isEditing ? (
                    <div>
                      ...
                      <p className="text-[10px] text-amber-600 mt-1.5 font-medium leading-tight">Attention: Changer ce prix recalculera le "Reste à payer" du client.</p>
                    </div>
                  ) : (
