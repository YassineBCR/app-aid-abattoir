...
import { enregistrerMouvement, paiementDejaEnregistre } from "../lib/comptabilite";
...
export default function PaiementReussi() {
  ...
  useEffect(() => {
    const validateAndFetch = async () => {
      ...
      try {
        ...
        // ── Création de la réservation après paiement ────────────────────
        ...
        if (panierId) queryTickets = queryTickets.eq('panier_id', panierId);
        ...
        if (ticketsToPay && ticketsToPay.length > 0) {
          for (const ticket of ticketsToPay) {
            ...
            const refUniqueParTicket = sessionId
              ...
              : `web_${ticket.id}_${Date.now()}`;

            // ── Vérification anti-doublon ──
            const dejaEnregistre = await paiementDejaEnregistre(refUniqueParTicket);
            ...
            try {
              await enregistrerMouvement({
                ...
                moyen_paiement:    'stripe_web',
                commande_id:       ticket.id,
                ticket_num:        ticket.ticket_num,
                caisse_id:         null,     // paiement web, pas de caisse physique
                reference_externe: refUniqueParTicket,
                notes:             `Acompte web — ${ticket.sacrifice_name || ''} — Session Stripe: ${sessionId || 'N/A'}`,
              });
            } catch (comptaErr) {
              // On log mais on ne bloque pas l'expérience client
              console.error(`Erreur comptabilité ticket ${ticket.ticket_num}:`, comptaErr);
            }
          }
        }

        // ── Récupération des tickets validés pour affichage ──────────────
        let query = supabase
          .from('commandes')
  ...
  useEffect(() => {
    ...
    const envoyerEmails = async () => {
      ...
      if (alreadySent) { setEmailsSent(true); return; }
  ...
  const downloadQR = (ticketNum) => {
    ...
    if (canvas) {
      ...
      link.href     = pngUrl;
      link.download = `Grammont_Ticket_${ticketNum}.png`;
