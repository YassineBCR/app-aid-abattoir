...
export default function PriseEnCharge() {
  ...
  const handlePaiement = async (e) => {
    ...
    try {
      const { error } = await supabase.rpc("ajouter_encaissement", {
  ...
  const getPaymentIcon = (moyen) => {
      switch(moyen) {
          ...
          case 'stripe_online': return <FiGlobe />;
  ...
  const getPaymentLabel = (moyen) => {
      switch(moyen) {
          ...
          case 'stripe_online': return 'Paiement Web';
              ...
              <div className="p-6 space-y-5">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <p className="font-mono font-medium text-slate-600 dark:text-slate-300">{commande.contact_phone || "-"}</p>
                    </div>
                    <div>
                        <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Créneau</p>
                        <p className="font-medium text-slate-600 dark:text-slate-300 whitespace-nowrap overflow-hidden text-ellipsis">
                            {commande.creneaux_horaires?.heure_debut ? commande.creneaux_horaires.heure_debut.slice(0,5) : "?"}
                        </p>
                    </div>
                </div>

                <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-2xl border border-slate-100 dark:border-slate-700">
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Sacrifice / Note</p>
                    <p className="text-sm font-medium text-slate-600 dark:text-slate-300 italic">
                        {commande.sacrifice_name || "Aucune note spécifique."}
                    </p>
                </div>
              </div>
            </div>
            ...
            <>
              ...
              <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-xl shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-slate-700 overflow-hidden flex flex-col md:flex-row">
                ...
                <div className="md:w-1/3 bg-slate-50 dark:bg-slate-900/80 p-6 md:p-8 flex flex-col justify-between border-r border-slate-200 dark:border-slate-700">
                    <div className="space-y-6">
                        <div>
                            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Prix Total</p>
                            <p className="text-2xl font-bold text-slate-800 dark:text-white">{total.toFixed(2)} €</p>
                        </div>
                        <div>
                            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Déjà versé</p>
                            <div className="flex items-center gap-2 mt-1 text-emerald-600 dark:text-emerald-400">
                                <FiCheckCircle />
                                <p className="text-xl font-bold">{dejaPaye.toFixed(2)} €</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="mt-8 pt-8 border-t border-slate-200 dark:border-slate-700">
                        ...
                        <p className={`text-4xl font-black tracking-tight ${isPaye ? "text-emerald-500" : "text-slate-900 dark:text-white"}`}>
                            {isPaye ? "OK" : <span>{resteAPayer.toFixed(2)}<span className="text-lg text-slate-400 ml-1">€</span></span>}
                        </p>
                    </div>
                </div>
                    ...
                    {!isPaye ? (
                        <form onSubmit={handlePaiement} className="h-full flex flex-col justify-between space-y-6">
                                ...
                                <div className="grid grid-cols-3 gap-3">
                                    {['especes', 'cb', 'cheque'].map(mode => (
                                        <button
                                            ...
                                            className={`relative py-4 px-2 rounded-2xl border-2 transition-all flex flex-col items-center justify-center gap-2 ${
                                                modePaiement === mode 
                                                ? "border-indigo-600 bg-indigo-50 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300 dark:border-indigo-500 shadow-md ring-1 ring-indigo-200 dark:ring-indigo-800" 
                                                : "border-slate-100 dark:border-slate-700 text-slate-500 hover:border-slate-300 dark:hover:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-800"
                                            }`}
                                        >
                                            <div className="text-2xl">{getPaymentIcon(mode)}</div>
                                            <span className="text-xs font-bold uppercase">{mode === 'cb' ? 'Carte' : mode}</span>
                                            {modePaiement === mode && <div className="absolute top-2 right-2 w-2 h-2 bg-indigo-600 rounded-full"></div>}
                                        </button>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div className="relative group">
                                    ...
                                    <input
                                        ...
                                        className="block w-full pl-12 pr-28 py-5 bg-slate-50 dark:bg-slate-900 border-2 border-slate-200 dark:border-slate-700 rounded-2xl text-3xl font-bold text-slate-900 dark:text-white placeholder-slate-300 focus:border-indigo-500 focus:bg-white dark:focus:bg-slate-800 outline-none transition-all"
                                        placeholder="0.00"
                                    />
                                    <button 
                                        type="button" 
                                        onClick={() => setMontantEncaisse(resteAPayer.toFixed(2))} 
                                        className="absolute right-3 top-3 bottom-3 px-4 bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-xl text-xs font-bold text-indigo-600 dark:text-indigo-300 hover:bg-indigo-50 dark:hover:bg-slate-600 transition-colors uppercase tracking-wider shadow-sm"
                                    >
                                        Le Solde
                                    </button>
                                </div>
                            ...
                            <button
                                ...
                                className="w-full py-5 bg-emerald-500 hover:bg-emerald-600 text-white font-bold rounded-2xl shadow-xl shadow-emerald-500/20 text-lg flex items-center justify-center gap-3 transition-all transform active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                                {loadingPaiement ? (
                                    <div className="w-6 h-6 border-4 border-white/30 border-t-white rounded-full animate-spin" />
                                ) : (
                                    <>
                                        <span className="uppercase tracking-wide">Confirmer l'encaissement</span>
                        ...
                        <div className="h-full flex flex-col items-center justify-center text-center space-y-4 py-12">
                            <div className="w-24 h-24 bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 rounded-full flex items-center justify-center animate-bounce-slow">
              ...
              <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-xl shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-slate-700 p-6 md:p-8">
                ...
                {historiquePaiements.length === 0 ? (
                    <div className="text-center py-8 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-dashed border-slate-200">
                        <p className="text-slate-400 italic">Aucun mouvement enregistré.</p>
                    </div>
                ) : (
                            ...
                            <div key={p.id} className="group flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-900/30 hover:bg-slate-100 dark:hover:bg-slate-900/80 rounded-2xl border border-slate-100 dark:border-slate-700 transition-colors">
                                <div className="flex items-center gap-4">
                                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-xl shadow-sm ${
                                        p.moyen === 'especes' ? 'bg-emerald-100 text-emerald-600' : 
                                        p.moyen === 'stripe_online' ? 'bg-blue-100 text-blue-600' :
                                        'bg-indigo-100 text-indigo-600'
                                    }`}>
                                        {getPaymentIcon(p.moyen)}
                                    </div>
                                    <div>
                                        <p className="font-bold text-slate-700 dark:text-slate-200 text-sm md:text-base capitalize">
                                            {getPaymentLabel(p.moyen)}
                                        </p>
                                        <p className="text-xs text-slate-400 font-medium mt-0.5">
                                            {new Date(p.created_at).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', hour: '2-digit', minute:'2-digit' })}
                                        </p>
                                    </div>
                                </div>
                                <div className="flex items-center gap-4">
